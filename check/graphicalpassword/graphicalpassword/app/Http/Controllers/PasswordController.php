...
class PasswordController extends Controller
{
    public function index(Request $request)
    {
        ...
        $failed_logins=admin_failedlogin::where('username',$request->username)->first();
        // dd($request->username);
        if ($failed_logins) {
            if(($failed_logins->failed_attempts)>2){

                return view('incorrect')->with([
                    "message"=>"your account in locked contact manager"
                ]);
            }else{
    ...
    public function select_reset_password(Request $request)
    {
        admin_resetpassword::create([
            'username' => $request->username,
            'email' => $request->email,
    ...
    public function select_password(Request $request)
    {
        ...
        //     "contact" => "Required |min:8| integer"
        // ]);
        if(strlen($request->contact)<8){
            return view('password.validation')->with([
                'message' => 'Contact number should be equal or greater than 8',
                'username' => $request->username,
            'email' => $request->email,
            'address' => $request->address,
            'city' => $request->city,
